...
public class AdditionalServiceForKilometer {
    ...
    public List<CarDto> listByKilometer(Optional<String> kilometer) {
        ...
        return listAllCars().stream().filter(e -> kilometer.isEmpty() || (e.getKilometers()
                >= parametersInInt[0] && e.getKilometers() <= parametersInInt[1])).collect(Collectors.toList());
    }
}
